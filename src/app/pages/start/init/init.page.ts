...
export class InitPage {
    ...
    public aufnehmerNameForSubtitle: Signal<string> = computed(() =>
        this.app.aufnehmer()
            ? `${this.app.aufnehmer()?.vorname} ${
                  this.app.aufnehmer()?.nachname
              }`
            : 'nicht ausgewählt'
    );
    public deviceNameForSubtitle: Signal<string> = computed(() =>
        this.app.deviceName() ? this.app.deviceName() : 'Der Gerätename fehlt!'
    );
    public dataLastSyncedForSubtitle =
    ...
    public druckerAvailabilityStatusForSubtitle = computed(() =>
        this.availability.druckerGrossAvailability()
            ? 'Alle Drucker erreichbar!'
            : 'Fehler bei den Drucker-Verbindungen'
    );

    public isAufnehmerSelected = computed(() =>
        this.app.aufnehmer() ? true : false
    );
    public isDeviceNameSet = computed(() =>
        this.app.deviceName() ? true : false
    );
    public isLookupDataAvailable =
